...
function IncomingModal({ booking, onAccept, onReject }: { booking: any; onAccept: () => void; onReject: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-background">
      <div className="flex items-center justify-between border-b border-border bg-primary-soft px-4 py-4 text-primary">
        <div className="flex items-center gap-2">
          <Car className="h-6 w-6 animate-pulse" />
          <span className="text-lg font-extrabold uppercase tracking-wide">New Ride Request</span>
        </div>
        <span className="rounded-full bg-rose-100 px-3 py-1 text-sm font-extrabold text-rose-700">{remaining}s</span>
      </div>

      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        <div className="rounded-2xl border-2 border-primary/20 bg-card p-4">
          <div className="text-center">
            <div className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Estimated Fare</div>
            <div className="mt-1 text-4xl font-extrabold text-primary">₹{booking.fare}</div>
            <div className="text-xs text-muted-foreground">
              {booking.trip_type} · {Number(booking.distance_km).toFixed(1)} km
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
          <div className="flex items-start gap-3">
            <span className="mt-1 grid h-6 w-6 place-items-center rounded-full bg-emerald-100">
              <MapPin className="h-3.5 w-3.5 text-emerald-700" />
            </span>
            <div className="min-w-0 flex-1">
              <div className="text-[10px] font-bold uppercase text-emerald-700">Pickup</div>
              <div className="text-sm font-semibold">{booking.pickup_address}</div>
            </div>
          </div>
          <div className="ml-3 h-4 w-px border-l-2 border-dashed border-muted-foreground/40" />
          <div className="flex items-start gap-3">
            <span className="mt-1 grid h-6 w-6 place-items-center rounded-full bg-rose-100">
              <MapPin className="h-3.5 w-3.5 text-rose-700" />
            </span>
            <div className="min-w-0 flex-1">
              <div className="text-[10px] font-bold uppercase text-rose-700">Drop</div>
              <div className="text-sm font-semibold">{booking.drop_address}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-border bg-card p-4"
        ...
        <div className="grid grid-cols-2 gap-3">
          <button onClick={handleReject} className="rounded-2xl bg-rose-600 py-4 text-base font-bold text-white">Reject</button>
          <button onClick={handleAccept} className="rounded-2xl bg-emerald-600 py-4 text-base font-bold text-white">Accept</button>
        </div>
      </div>
    </div>
  );

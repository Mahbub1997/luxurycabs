...
import { Plus, Wallet, X, Loader2, Check, XCircle } from "lucide-react";
import { createDriverAccount, decideWithdrawal, updateDriverStatus } from "@/lib/admin.functions";
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {drivers.length === 0 && <p className="text-sm text-muted-foreground">No drivers yet. Add one.</p>}
      </div>

      {creating && <CreateDriverModal onClose={() => { setCreating(false); load(); }} />}
...
function CreateDriverModal({ onClose }: { onClose: () => void }) {
  const [f, setF] = useState({
    ...
    vehicle_type: "sedan", vehicle_model: "", vehicle_number: "", license_number: "",
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await createDriverAccount({ data: f });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center">
      <form onSubmit={submit} className="w-full max-w-md rounded-t-3xl bg-card p-5 sm:rounded-3xl">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-lg font-bold">Add Driver</h3>
          <button type="button" onClick={onClose}><X className="h-4 w-4" /></button>
        </div>
        <div className="grid grid-cols-2 gap-2">
          {(["email", "password", "name", "phone", "vehicle_type", "vehicle_model", "vehicle_number", "license_number"] as const).map((k) => (
            <input
              key={k}
              required={k === "email" || k === "password" || k === "name" || k === "phone" || k === "vehicle_type"}
        ...
        <button disabled={busy} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-bold text-primary-foreground">
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Create driver"}
        </button>
      </form>
    </div>
  );
            ...
            <div key={r.id} className="rounded-lg border border-border p-2 text-xs">
              ...
              {r.status === "pending" && (
                <div className="mt-2 flex gap-1">
                  <button
                    onClick={async () => {
                      try { await decideWithdrawal({ data: { request_id: r.id, approve: true } }); toast.success("Approved"); load(); } catch (e: any) { toast.error(e.message); }
                    }}
                  ...
                  <button
                    onClick={async () => {
                      try { await decideWithdrawal({ data: { request_id: r.id, approve: false } }); toast.success("Rejected"); load(); } catch (e: any) { toast.error(e.message); }
                    }}

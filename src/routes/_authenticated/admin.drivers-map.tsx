  ...
  useEffect(() => {
    (async () => {
      ...
      for (const d of drivers) {
        ...
        bounds.extend(pos);
        any = true;
        const label = d.vehicle_number || d.name || "—";
        const icon: google.maps.Symbol = {
          path: "M -10 -6 L 10 -6 L 12 0 L 10 6 L -10 6 L -12 0 Z",
          fillColor: d.is_online ? "#16a34a" : "#64748b",
          fillOpacity: 1,
          strokeColor: "#ffffff",
          strokeWeight: 1.5,
          scale: 1.2,
        ...
        if (existing) {
          ...
          existing.setLabel({ text: label, color: "#0f172a", fontSize: "10px", fontWeight: "700" });
        } else {
          const m = new g.maps.Marker({
            map: mapRef.current,
            position: pos,
            icon,
            label: { text: label, color: "#0f172a", fontSize: "10px", fontWeight: "700" },
            title: `${d.name} · ${d.vehicle_model ?? ""} ${d.vehicle_number ?? ""}`,
          });
